...
test('Resuelve un objeto con la información de cada link', () => {
  expect.assertions(1);
  return mdLinks('src/prueba2.md').then(data => {
    expect(data).toEqual([ { href: 'https://nodejs.org/api/fs.html#fs_fs_readdir_path_options_callback',
...
test('Resuelve un objeto con las estadisticas de los links analizados en el archivo: Total de links, links unicos y links validos', async () => {
  await expect
...
test(`Entrega un objeto con la camtidad de links que posee el directorio y cuantos de ellos son unicos y cuantos estan rotos`, () => {
  expect.assertions(1);
